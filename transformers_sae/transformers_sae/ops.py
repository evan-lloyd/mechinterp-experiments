...
def find_checkpoint_after(
    checkpoint_dir: str, layer: int, min_tokens: int
) -> Tuple[str | None, int | None]:
    """Find the checkpoint with the smallest number of tokens greater than min_tokens for the given layer.

    Returns the path to the checkpoint file, or None if no such checkpoint exists.
    """
    best_path: str | None = None
    ...
    for filename in os.listdir(checkpoint_dir):
        ...
        if parsed is None or parsed[0] != layer:
            continue
        _, tokens = parsed
        if tokens > min_tokens and (best_tokens is None or tokens < best_tokens):
            best_tokens = tokens
            best_path = os.path.join(checkpoint_dir, filename)
...
def get_state_dict_from_checkpoint(in_file: str):
    with tempfile.TemporaryDirectory() as tmpdir:
        # Extract zip file
        ...
        with safe_open(
            os.path.join(tmpdir, "sae_weights.safetensors"), framework="pt"
        ) as f:
