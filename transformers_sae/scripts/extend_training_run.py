...
for training_method in args.training_methods:
    config_for_run = deepcopy(training_config)

    if "finetuned" in training_method:
        raise NotImplementedError(f"Not implemented: {training_method}")
    if training_method.startswith("next_layer"):
        config_for_run.method = TrainingMethod.next_layer
    elif training_method.startswith("standard"):
        config_for_run.method = TrainingMethod.standard
    else:
        raise NotImplementedError(f"Not implemented: {training_method}")

    print(f"Extending training run for {training_method}")
    ...
    training_results = train(
        ...
        checkpoints_at=list(
            range(
                int(1e7),
                config_for_run.num_train_tokens,
                int(1e7),
            )
        ),
        checkpoint_dir=f"{CHECKPOINT_BASE_PATH}/{training_method}",
        extend_training_run=True,
        skip_kl_eval=True,
        offload_after_training=True,
    )
    saes = training_results.final_saes
