...
class EndToEndTrainingStepper(Stepper):
    ...
    def run_layers(self) -> List[int]:
        return list(range(self.target_layer, self.base_model.config.num_layers + 1))
    ...
    def step(
        ...
        if config.balance_reconstruction_losses[self.target_layer]:
            if num_downstream_layers == 0:
                ...
                mse_scale = (
                    downstream_reconstruction_loss.item() / num_downstream_layers
                )
            kl_scale *= mse_scale / (downstream_reconstruction_loss.item() + 1e-8)
