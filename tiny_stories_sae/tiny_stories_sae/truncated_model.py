...
from .replacement_model import ReplacementModel, GenericReplacementLayer


@contextmanager
def truncated_model(
    model: torch.nn.Module, start_layer: int, end_layer: int, start_at_sae: bool
):
    ...
    if (
        start_layer == -1
        or start_layer >= model.config.num_layers
        or not isinstance(model, ReplacementModel)
    ) and start_at_sae:
        raise ValueError(
